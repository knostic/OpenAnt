    ...
    while budget.turns < MAX_TURNS:
        ...
        for block in assistant_content:
            ...
            if block.name == finish_tool.name:
                ...
                # skip it so the loop nudges for a complete answer (and, if it never
                # completes, exhausts MAX_TURNS and raises — a visible failure, not a
                # silent partial). Mirrors the verifier + enhancer max_tokens gate.
