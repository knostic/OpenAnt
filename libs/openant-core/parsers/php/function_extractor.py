    ...
    def _extract_functions_from_tree(self, tree, source: bytes, file_path: Path,
        ...
        while stack:
            ...
            if node.type == 'function_definition':
                ...
                # Recurse into the body so nested function definitions
                # (which become global once the outer function runs) are
                # also extracted. A nested function has no enclosing class.
                ...
                # Recurse into the body so a function defined inside a method
                # body is extracted too (it becomes global, no class context).
                body_node = node.child_by_field_name('body')
                if body_node:
                    for child in reversed(body_node.children):
                        stack.append((child, None, namespace_name))
