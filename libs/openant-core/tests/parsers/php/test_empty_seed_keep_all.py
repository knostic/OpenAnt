"""Bug (N4): the PHP reachability filter silently blacks out a no-entry-point
dataset.
...
(PHP) This drives the REAL `apply_reachability_filter` (real EntryPointDetector + real
...
_TP = _CORE / "parsers" / "php" / "test_pipeline.py"
...
def _load_pipeline():
    ...
    for p in (str(_TP.parent), str(_CORE)):
        if p not in sys.path:
            sys.path.insert(0, p)
    spec = importlib.util.spec_from_file_location("isolated_php_test_pipeline", _TP)
...
_FUNCTIONS = {
    "lib.php:add": {"name": "add", "unit_type": "function", "file_path": "lib.php", "code": "return a + b"},
    "lib.php:scale": {"name": "scale", "unit_type": "function", "file_path": "lib.php",
                    "code": "return add(a, a)"},
}
_UNITS = [
    {"id": "lib.php:add",
     "metadata": {"direct_calls": [], "direct_callers": ["lib.php:scale"]}},
    {"id": "lib.php:scale",
     "metadata": {"direct_calls": ["lib.php:add"], "direct_callers": []}},
]
...
def _make_pipeline():
    ...
    p = mod.PHPPipelineTest(repo_path=str(tmp))
...
def test_empty_seed_keeps_all_units():
    ...
    assert p.reachable_units == {"lib.php:add", "lib.php:scale"}, (
...
def test_written_dataset_retains_every_unit_at_zero_reduction():
    ...
    assert ids == {"lib.php:add", "lib.php:scale"}, "units were dropped (blackout)"
