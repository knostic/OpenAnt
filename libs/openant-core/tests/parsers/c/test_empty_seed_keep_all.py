"""Bug (N4): the C reachability filter silently blacks out a no-entry-point
dataset.
...
This drives the REAL `apply_reachability_filter` (real EntryPointDetector + real
...
_TP = _CORE / "parsers" / "c" / "test_pipeline.py"
...
def _load_pipeline():
    ...
    spec = importlib.util.spec_from_file_location("isolated_c_test_pipeline", _TP)
...
_FUNCTIONS = {
    "lib.c:add": {"name": "add", "unit_type": "function", "file_path": "lib.c",
                  "code": "int add(int a, int b) { return a + b; }"},
    "lib.c:scale": {"name": "scale", "unit_type": "function", "file_path": "lib.c",
                    "code": "int scale(int a) { return add(a, a); }"},
}
_UNITS = [
    {"id": "lib.c:add",
     "metadata": {"direct_calls": [], "direct_callers": ["lib.c:scale"]}},
    {"id": "lib.c:scale",
     "metadata": {"direct_calls": ["lib.c:add"], "direct_callers": []}},
]
...
def _make_pipeline():
    ...
    p = mod.CPipelineTest(repo_path=str(tmp))
...
def test_empty_seed_keeps_all_units():
    ...
    assert p.reachable_units == {"lib.c:add", "lib.c:scale"}, (
...
def test_written_dataset_retains_every_unit_at_zero_reduction():
    ...
    assert ids == {"lib.c:add", "lib.c:scale"}, "units were dropped (blackout)"
