...
def _run_fallback(**overrides):
    kwargs = dict(
        ...
        investigation_context=None, planner_evidence_ctx="original evidence",
...
class TestRunEvidenceGapStrategyFallback:
    ...
    def test_identical_evidence_skips_rerun_deterministically(self):
        """Byte-for-byte identical text -- the larger budget genuinely
        found nothing new -- must not trigger a pointless second Strategy
        call. This is the exact, sole mechanism 'new evidence actually
        acquired' is determined by."""
        with (
            mock.patch(
                "utilities.autopatcher.remediation_planner.build_planner_evidence",
                return_value="original evidence",
            ) as spy_evidence,
            mock.patch("utilities.autopatcher.remediation_planner.generate_remediation_strategy") as spy_strategy,
        ):
            result = _run_fallback(planner_evidence_ctx="original evidence")
    ...
    def test_empty_new_evidence_skips_rerun(self):
        with (
            mock.patch("utilities.autopatcher.remediation_planner.build_planner_evidence", return_value=""),
            mock.patch("utilities.autopatcher.remediation_planner.generate_remediation_strategy") as spy_strategy,
        ):
            result = _run_fallback()
        spy_strategy.assert_not_called()
    ...
    def test_different_evidence_calls_strategy_exactly_once(self):
    ...
    def test_no_synthetic_strategy_result_when_generate_strategy_never_called(self):
        """If no new evidence is acquired, generate_remediation_strategy is
    ...
    def test_budget_controller_extension_is_respected(self):
        ...
        budget_controller already allows."""
