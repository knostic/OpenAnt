...
def step_context(step: str, output_dir: str, inputs: dict | None = None):
    ...
    # #625: the dropped-block baseline for the per-step DELTA — the
    # process-global counter is run-cumulative; this step's report must
    # carry only what THIS step dropped (a raw snapshot would re-report
    # every earlier step's drops, and summing those across steps
    # overcounts by the prefix).
    from utilities.llm_client import get_dropped_block_totals
    _start_dropped = dict(get_dropped_block_totals() or {})
    ...
    try:
        ...
        if _accounting_error:
            ...
            # DELTA must surface HERE too (an accounting error and
            # dropped blocks are not mutually exclusive). The counter is
            # in-process, so the delta is available even when the
            # tracker's token baseline is not.
            from utilities.llm_client import get_dropped_block_totals
            _dropped = _dropped_delta(get_dropped_block_totals(), _start_dropped)
            if _dropped:
                report.token_usage["dropped_blocks"] = _dropped
        else:
            ...
            if _accounting_error_count():
                ...
                report.token_usage["cost_incomplete"] = True
            # #625: the per-kind dropped-block DELTA reaches the
            # persisted surface (the tracker's call records die with the
            # process; the step report is where the reconciliation lives)
            # — only the kinds THIS step dropped, present-only, the same
            # discipline as accounting_error.
            from utilities.llm_client import get_dropped_block_totals
            _dropped = _dropped_delta(get_dropped_block_totals(), _start_dropped)
            if _dropped:
                report.token_usage["dropped_blocks"] = _dropped
...
def _dropped_delta(end: dict | None, start: dict) -> dict:
    """#625: the per-step delta of the run-cumulative dropped-block
    counter — only kinds that increased THIS step, never the prefix (a
    later step must not re-report an earlier step's drops; a counter
    reset mid-step reads as the fresh count, never negative)."""
    if not end:
        return {}
    delta: dict = {}
    for kind, count in end.items():
        d = count - start.get(kind, 0)
        if d > 0:
            delta[kind] = d
    return delta
