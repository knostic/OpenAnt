...
- **The excluded-directory count is no longer nameless.** A directory-name
  prune now records a name-keyed histogram (`excluded_dir_names`) with 1-2
  entry-relative example paths per retained name and an overflow disclosure
  (`excluded_dir_names_overflow`) — retention reserved for the scanner's
  effective exclusion set (the first-party names: `build/`, `env/`,
  `migrations/` …), dynamic names bounded with the overflow disclosed as an
  occurrence count; hostile/oversized/non-ASCII names count but never key
  the artifact. Seven scanners instrumented (python, c, php, ruby, rust,
  swift, zig); JS remains count-only (camelCase alias) and Go's pipeline
  mode writes no scan-result artifact — both disclosed, never zeroed.
  What is excluded is unchanged; only what the artifacts record.
- **The counters reach the artifacts operators read.** The discovery block
  ...
  failed language is disclosed and its stale artifact never read; a reused
  output directory's newest artifact wins by mtime, an mtime tie disclosed
  rather than guessed.
