...
For those who prefer a browser over the CLI, `openant serve` starts a local web
UI that drives the same scan pipeline — submit a repository URL or local path,
watch the scan logs stream live, and read the HTML report, markdown summary, and
disclosures:
