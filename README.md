...
Phases accept an optional `thinking` key — the request-side thinking policy, in the SDK's own dict shape (`{"type": "adaptive"}`, or `{"type": "enabled", "budget_tokens": N}` with N an integer >= 1024 and < 4096, or `{"type": "disabled"}`). Absent (the default) sends no thinking key at all — the instrument stays byte-identical; an explicit `disabled` is a distinct request. Consumed by the anthropic provider; other types warn loudly once. The `enhance`/`verify` phases (tool-calling loops) reject it at registry build (a loud startup error) — the platform requires thinking blocks echoed back with tool results, which this pipeline does not yet carry; the same guard fires at request time for any other tool-carrying call (including the threat-model repository exploration). A non-default policy is recorded in the step reports, folded into the analyze/verify/llm-reachability checkpoint identities (a policy change re-pays those phases' checkpoints), and the run's per-kind dropped-block counts (thinking, refusal, ...) surface in the step reports' `dropped_blocks` for the delivered-vs-billed reconciliation.
